...
impl GrContext {
    ...
    pub fn set_resource_cache_limit(&self, max_resources: i32, max_resources_bytes: usize) {
        unsafe {
            gr_context_set_resource_cache_limits(
                self.raw_pointer,
                max_resources,
                max_resources_bytes,
            )
    ...
    pub fn get_max_surface_sample_count_for_color_type(&self, colorType: ColorType) -> i32 {
        unsafe {
            gr_context_get_max_surface_sample_count_for_color_type(self.raw_pointer, colorType)
        }
    }
